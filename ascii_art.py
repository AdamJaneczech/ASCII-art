...
def get_char_for_value(val, min_val, max_val, char_list, char_grayscale):
    ...
    #Index for char_list
    ind = int(normalized_val * (len(char_list) - 1))
    return char_list[ind]

def load_and_convert_image(image_path, patch_width):
    img = pg.image.load(image_path)
    img_array = pg.surfarray.array3d(img)
    
    gray_img = np.dot(img_array[...,:3], [0.3, 0.59, 0.11])
    
    img_width, img_height = gray_img.shape[:2]
    ...
    for y in range(0, img_height, patch_height):
        for x in range(0, img_width, patch_width):
            patch = gray_img[x:min(x+patch_width, img_width), y:min(y+patch_height, img_height)]
            avg_gray_value = np.mean(patch)
            char = get_char_for_value(avg_gray_value, 0, 255, characters, grayscale)
            ascii_art += char
        ascii_art += "\n"
...
pg.init()
image_path = "swarm_of_drones.jpg"
patch_width = 3
ascii_art = load_and_convert_image(image_path, patch_width)
save_ascii_art(ascii_art, "ascii_img.txt")
print(ascii_art)